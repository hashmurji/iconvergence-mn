// api/transactions.js
import { pool } from "../lib/db.js";
import { requireAuth, resolveClientScope } from "../lib/auth.js";

export default async function handler(req, res) {
  if (req.method !== "GET") return res.status(405).json({ error: "Method not allowed" });

  try {
    const auth = await requireAuth(req);
    const scopedClientId = resolveClientScope(auth, req.query.clientId);
    const limit = Math.min(parseInt(req.query.limit, 10) || 500, 2000);
    const offset = parseInt(req.query.offset, 10) || 0;

    let query = `SELECT * FROM transactions`;
    const params = [];
    if (scopedClientId) {
      params.push(scopedClientId);
      query += ` WHERE client_id = $${params.length}`;
    }
    query += ` ORDER BY trade_date DESC`;
    params.push(limit);
    query += ` LIMIT $${params.length}`;
    params.push(offset);
    query += ` OFFSET $${params.length}`;

    const { rows } = await pool.query(query, params);

    const txns = rows.map((r) => ({
      txRef: r.tx_ref,
      clientId: r.client_id,
      primaryClientCode: r.primary_client_code,
      selector: r.selector,
      tradeDate: r.trade_date,
      tradedate: r.trade_date,
      settlementDate: r.settlement_date,
      settdate: r.settlement_date,
      financialAccountNumber: r.financial_account_number,
      accountId: r.financial_account_number,
      financialAccountName: r.financial_account_name,
      txType: r.txtype,
      txtype: r.txtype,
      ticker: r.ticker,
      portfolioType: r.portfolio_type,
      description: r.description,
      currency: r.currency,
      ccy: r.currency,
      quantity: r.quantity,
      qty: r.quantity,
      consideration: parseFloat(r.consideration) || 0,
      tx10bps: r.tx10bps,
      commission: r.commission,
      clientNetAmount: parseFloat(r.client_net_amount) || 0,
      clientnetamt: parseFloat(r.client_net_amount) || 0,
      costPrice: r.cost_price,
      costValue: r.cost_value,
      postReference: r.post_reference,
    }));

    res.status(200).json({ txns });
  } catch (err) {
    res.status(err.status || 500).json({ error: err.message });
  }
}
